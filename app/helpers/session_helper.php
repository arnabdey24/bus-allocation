...
function restoreSessionIfAvailable()
{
    if (!isset($_SESSION['user_id'])) {
        if (isset($_COOKIE['nstu-bus-allocation-session-data'])) {
            $data = json_decode($_COOKIE['nstu-bus-allocation-session-data']);
...
function sendMail($edu_mail, $link)
{
    ...
    $mail->Username = 'busallocation.system.nstu@gmail.com';       // SMTP username
    $mail->Password = 'adjbhrfscyzfltfo';         // SMTP password
    $mail->SMTPSecure = 'ssl';                  // Enable TLS encryption, `ssl` also accepted
    $mail->Port = 465;                          // TCP port to connect to

    // Sender info
    $mail->setFrom('busallocation.system.nstu@gmail.com', 'BusAllocationSystem@NSTU');
    ...
    // Mail subject
    $mail->Subject = 'Verify your BusAllocationSystem@NSTU account';

    // Mail body content
    $bodyContent = '<h1>To verify your <a href="http://localhost/bus-allocation">BusAllocationSystem@NSTU</a> account please click on the below link:</h1>';
